...
//bookmark----
#define SWITCH_BETWEEN_NEW_AND_OLD_AVOIDANCE
#define USE_SMALLER_STOP_DISTANCE_NEAR_POSTS
...
void ControlModule::generatePaths() 
{
  ...
  float distance, leftPostDistance, rightPostDistance;
  ...
  for (RobotIndex index=ROBOT0; index<NUM_ROBOTS; index++) 
  {
    ...
    leftPostDistance = 
      dist( start.xPos, start.yPos, SP->field.OUR_GOAL_LINE, SP->field.LEFT_GOAL_POST);
    rightPostDistance = 
      dist( start.xPos, start.yPos, SP->field.OUR_GOAL_LINE, SP->field.RIGHT_GOAL_POST);


#ifdef USE_SMALLER_STOP_DISTANCE_NEAR_POSTS
    // Are we close to the posts and close enough to stop?
    if ( ( (leftPostDistance  < 2*SP->general.PLAYER_RADIUS) ||
           (rightPostDistance < 2*SP->general.PLAYER_RADIUS) ) &&
         ( distance < typeParams->STOP_DISTANCE )
       )
    {
      // Use a tighter stop criterion when we are close to the posts
      ...
      // Otherwise, use proportional gain?
      else
      {
    ...
    if (false) { }
#endif
    // Are we far from the posts and close enough to stop?
    else if ( distance < typeParams->STOP_DISTANCE )
    {
      robotPath[index].clearPath();
...
#ifdef SWITCH_BETWEEN_NEW_AND_OLD_AVOIDANCE
        if (start.xPos < ((SP->field.HALF_LINE + SP->field.THEIR_GOAL_LINE)/2.0f))
        {
          findOpenPath(
            start,
            final,
