#include "jamAndShoot.h"
#include "../basic_actions.h"
Play JamAndShoot::playNum = -1;



JamAndShoot::JamAndShoot(VisionModule *vm) : BasePlay(vm)
{
   loadValues();
}
...
//*********************************************************
// Execute Play
void JamAndShoot::executePlay(VisionData* vision, RobocupStrategyData* rsd)
{
  standardExecution(*vision, rsd);
}
//*********************************************************
// Get Action Function
ActionFunction JamAndShoot::getActionFunction(Position pos)
{
	switch(pos)
	{
  	case BLOCKER: return Blocker::jamAndShoot;  
  	case DEFENDER: return Defender::jamAndShoot;
	  case AGGRESSOR: return Aggressor::jamAndShoot;
  	case CREATOR: return Creator::jamAndShoot; 
...
void JamAndShoot::loadValues()
{
  ///Open the paramater file:
  ifstream mainFile;
  mainFile.open("Params/Play_Params/2002plays/JamAndShootParams.txt", ios::nocreate);  
